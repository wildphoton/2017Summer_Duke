...
sys.path.append(os.path.realpath('../../2017Summer/metrics'))

from seg_metric import SegMetric

FLAGS = tf.flags.FLAGS
tf.flags.DEFINE_integer("batch_size", "8", "batch size for training")
tf.flags.DEFINE_integer("visualize_size", "10", "batch size for visualizing results")
tf.flags.DEFINE_string("logs_dir", "logs_batch8/", "path to logs directory")
...
def main(argv=None):
    ...
    if FLAGS.mode == "train":
        ...
        valid_images, valid_annotations = validation_dataset_reader.next_batch(FLAGS.visualize_size)
        ...
        pred_valid = sess.run(pred_annotation, feed_dict={image: valid_images, annotation: valid_annotations,
                                                    keep_probability: 1.0})
        pred_train = sess.run(pred_annotation, feed_dict={image: train_images, annotation: train_annotations,
                                                          keep_probability: 1.0})
        ...
        """save images"""
        imageDir = FLAGS.logs_dir + "/images"
        if not os.path.isdir(imageDir):
            os.makedirs(imageDir)

        for itr in range(FLAGS.visualize_size):
            utils.save_image(valid_images[itr].astype(np.uint8), imageDir, name="inp_test" + str(itr))
            utils.save_image(valid_annotations[itr].astype(np.uint8), imageDir, name="gt_test" + str(itr))
            utils.save_image(pred_valid[itr].astype(np.uint8), imageDir, name="pred_test" + str(itr))
