...
class SegMetric:
    ...
    def add_image_pair(self, eval_segm, gt_segm):
        ...
        eval_mask, gt_mask = extract_both_masks(eval_segm, gt_segm, self.current_classes, self.current_num_classes)

        for i, c in enumerate(self.current_classes):
    ...
    def mean_accuracy(self):
        with np.errstate(divide='ignore', invalid='ignore'):
            mean_accuracy = sum_nan(self.n_ii / self.t_i) / self.current_num_classes
    ...
    def mean_IU(self):
        with np.errstate(divide='ignore', invalid='ignore'):
            mean_IU = sum_nan(self.n_ii / (self.t_i + self.n_ij - self.n_ii)) / self.current_num_classes
