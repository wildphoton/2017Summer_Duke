...
def image_scaling(img, label):
    ...
    scale = tf.random_uniform([1], minval=1, maxval=1.5, dtype=tf.float32, seed=None)
...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror, random_rotate, ignore_label,
...
class ImageReader(object):
    ...
    def __init__(self, data_list, input_size,
        ...
        Args:
          data_list: full path to the file with lines of the form '/path/to/image /path/to/mask'.
        ...
        self.image, self.label = read_images_from_disk(self.queue, self.input_size, random_scale, random_mirror,
                                                       random_rotate, ignore_label, img_mean)
    ...
    def dequeue(self, num_elements):
        ...
        image_batch, label_batch = tf.train.batch([self.image, self.label], num_elements, capacity=num_elements * 4,
                                                  allow_smaller_final_batch=False)
        return image_batch, label_batch

    def shuffle_dequeue(self, num_elements):
        '''Pack images and labels into a batch in random order.
        ...
        Returns:
          Two tensors of size (batch_size, h, w, {3, 1}) for images and masks.'''
        image_batch, label_batch = tf.train.shuffle_batch([self.image, self.label], batch_size=num_elements,
                                                          capacity=num_elements * 10,
                                                          min_after_dequeue=num_elements * 2,
                                                          allow_smaller_final_batch=False)
        return image_batch, label_batch
